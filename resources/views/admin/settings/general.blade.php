    ...
    <div class="col-12">
        <form role="form" id="createForm" method="POST" action="{{url('/admin/settings/update')}}"
            ...
            <div class="card-body">
                ...
                <x-form-elements name="app_name" type="text" label="App Name"
                    value="{{old('app_name',$settings->app_name)}}" />

                <div class="form-group row">
                    <label for="app_timezone" class="col-sm-3 col-form-label">Time Zone
                    </label>
                    <select name="app_timezone" class="col-sm-9 form-control select2">
                        @foreach ($timezones as $key => $option)
                        <option value="{{ $key }}"
                            {{ ($key == old('app_timezone',$settings->app_timezone) ) ? "selected":"" }}>
                            {{ $option }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group row">
                    <label for="currency_code" class="col-sm-3 col-form-label">Currency Code
                    </label>
                    <select name="currency_code" class="col-sm-9 form-control select2">
                        @foreach ($currencies as $option)
                        <option value="{{ $option }}"
                            {{ ($option == old('currency_code',$settings->currency_code) ) ? "selected":"" }}>
                            {{ $option }}</option>
                        @endforeach
                    </select>
                </div>

                <x-form-elements name="currency_symbol" type="text" label="Currency Symbol"
                    value="{{old('currency_symbol',$settings->currency_symbol)}}" />

                <hr />

                <x-form-elements name="tax_applicable" type="switch" label="Tax Applicable"
                    value="{{old('tax_applicable',$settings->tax_applicable)}}" />

                <x-form-elements name="tax_percentage" type="text" label="Tax Percentage"
                    value="{{old('tax_percentage',$settings->tax_percentage)}}" />

                <hr />

                <x-form-elements name="delivery_charge_applicable" type="switch" label="Delivery Charge Applicable"
                    value="{{old('delivery_charge_applicable',$settings->delivery_charge_applicable)}}" />

                <x-form-elements name="delivery_charge" type="text" label="Delivery Charge"
                    value="{{old('delivery_charge',$settings->delivery_charge)}}" />

                <input type="hidden" value="general" name="setting_tab" />
