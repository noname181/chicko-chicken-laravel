...
@section('title', 'Create a Addon')

@section('content_header')
<h1 class="m-0 text-dark">Addons</h1>
@stop
	...
	<div class="col-12">
		...
		<div class="card card-primary">
			<div class="card-header">
				<h3 class="card-title">Create a Addon</h3>
			</div>
			<!-- /.card-header -->
			<!-- form start -->
			<form role="form" id="createForm" method="POST" action="{{url('/restaurant-owner/dish_addons/store')}}" enctype="multipart/form-data">
				@csrf
				<div class="card-body">
					@include('errors.errors-forms')
					<x-form-elements name="name" type="text" label="Addon Name" value="{{old('name')}}" required />

					<x-form-elements name="addons_category_id" type="select2" label="Addon Category"
						value="{{old('addons_category_id')}}" :options="$categories" required />
